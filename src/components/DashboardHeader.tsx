...
import { Play, Square, Pause, HardDrive, Cpu, Film, Terminal, HelpCircle, Video, Loader2, Mic, MicOff, BookOpen, Globe } from 'lucide-react';
...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  const handleToggleCommentaryLang = () => {
    ...
    <header className="bg-slate-900 border-b border-slate-800 px-4 py-2.5 select-none">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-center">
          ...
          {/* 3,000 Cốt Truyện Kịch Bản Hài Hước */}
          {onOpenStorylines && (
            <button
              id="btn-open-storylines"
              onClick={onOpenStorylines}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gradient-to-r from-amber-500/20 via-rose-500/20 to-amber-500/20 hover:from-amber-500/30 hover:to-rose-500/30 border border-amber-500/50 text-xs font-bold text-amber-300 transition active:scale-95 cursor-pointer shadow-md shadow-amber-950/40"
              title="Mở Kho 3,000 Cốt Truyện Kịch Bản Hài Hước (60 Nhân Vật Phụ, 125 Sự Kiện Kích Động, 100 Khúc Cua, 100 Mức Phạt)"
            >
              <BookOpen className="w-4 h-4 text-amber-400" />
              <span>3,000 Cốt Truyện</span>
              <span className="px-1.5 py-0.5 rounded bg-amber-950 border border-amber-500/40 text-[10px] font-mono text-amber-300">
                PRO
              </span>
            </button>
          )}
          ...
          {/* Nút chuyển đổi ngôn ngữ bình luận VI / EN */}
          <button
            id="btn-toggle-commentary-lang"
            onClick={handleToggleCommentaryLang}
            className={`flex items-center gap-1 px-2.5 py-2 rounded-lg text-xs font-bold border transition active:scale-95 cursor-pointer ${
              commentaryLang === 'en'
                ? 'bg-sky-950/80 border-sky-500/50 text-sky-300 shadow-md shadow-sky-950/50'
                : 'bg-emerald-950/80 border-emerald-500/50 text-emerald-300 shadow-md shadow-emerald-950/50'
            }`}
            title="Chuyển đổi ngôn ngữ Bình Luận Viên: Tiếng Việt 🇻🇳 hoặc Tiếng Anh 🇬🇧"
          >
            <Globe className="w-3.5 h-3.5" />
            <span>{commentaryLang === 'en' ? 'BLV: EN 🇬🇧' : 'BLV: VI 🇻🇳'}</span>
          </button>
        </div>
