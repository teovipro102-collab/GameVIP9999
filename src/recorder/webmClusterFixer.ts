...
function concatBuffers(buffers: Uint8Array<any>[]): Uint8Array {
  let totalLen = 0;
  ...
  try {
    ...
    while (segPos < rawBuf.length) {
      ...
      if (idInfo.id === 0x1654AE6B) { // Tracks
        ...
        let tcVal = 0;
        let payloadRest: Uint8Array<any> = clusterPayload;
    ...
    return new Blob([finalBuffer as unknown as BlobPart], { type: blob.type || 'video/webm' });
  } catch (err) {
